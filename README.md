# Anurag Bonde

[![terminal-init](https://readme-typing-svg.herokuapp.com?font=JetBrains+Mono&weight=700&size=28&duration=1000&pause=500&color=00FF41&center=true&vCenter=true&width=600&lines=initializing+terminal...;loading+portfolio...;whoami;Anurag%24;ready+for+commands!;cloning+Anurag...;navigating+to+/projects...;executing:+passion+--all;fetching:+github_stats+--live;compiling:+goals.c;opening:+README.md)](https://github.com/Anuraggg-19)

```
> Problem Solver | CP Enthusiast | Full Stack Dev | 8.70 CGPA
> 2-Star CC | 330+ LC
> Mumbai - Available - Building stuff that matters
```

```
Navigate using the commands below (or just click the sections!):

whoami      → About Me
contact     → How to Reach Me
```

---

## ⚙️ Tech and Tools I Use

[![java](https://skillicons.dev/icons?i=java)](https://skillicons.dev/icons?i=java)
[![python](https://skillicons.dev/icons?i=python)](https://skillicons.dev/icons?i=python)
[![c](https://skillicons.dev/icons?i=c)](https://skillicons.dev/icons?i=c)
[![cpp](https://skillicons.dev/icons?i=cpp)](https://skillicons.dev/icons?i=cpp)
[![javascript](https://skillicons.dev/icons?i=javascript)](https://skillicons.dev/icons?i=javascript)
[![typescript](https://skillicons.dev/icons?i=typescript)](https://skillicons.dev/icons?i=typescript)
[![html](https://skillicons.dev/icons?i=html)](https://skillicons.dev/icons?i=html)
[![css](https://skillicons.dev/icons?i=css)](https://skillicons.dev/icons?i=css)
[![react](https://skillicons.dev/icons?i=react)](https://skillicons.dev/icons?i=react)
[![nodejs](https://skillicons.dev/icons?i=nodejs)](https://skillicons.dev/icons?i=nodejs)
[![express](https://skillicons.dev/icons?i=express)](https://skillicons.dev/icons?i=express)
[![mongodb](https://skillicons.dev/icons?i=mongodb)](https://skillicons.dev/icons?i=mongodb)
[![mysql](https://skillicons.dev/icons?i=mysql)](https://skillicons.dev/icons?i=mysql)
[![git](https://skillicons.dev/icons?i=git)](https://skillicons.dev/icons?i=git)
[![github](https://skillicons.dev/icons?i=github)](https://skillicons.dev/icons?i=github)
[![vscode](https://skillicons.dev/icons?i=vscode)](https://skillicons.dev/icons?i=vscode)
[![linux](https://skillicons.dev/icons?i=linux)](https://skillicons.dev/icons?i=linux)
[![tensorflow](https://skillicons.dev/icons?i=tensorflow)](https://skillicons.dev/icons?i=tensorflow)

---

## 💻 Interactive Terminal Commands

---

`whoami`

```python
# 👤 About Me - Developer Profile
class Developer:
    def __init__(self):
        self.name       = "Anurag Bonde"
        self.role       = "Full Stack Developer | DSA Enthusiast"
        self.education  = "DJ Sanghvi College of Engineering"
        self.cgpa       = 8.70
        self.location   = "📍 Mumbai, India"
        self.status     = "✅ Open to internships and full-time roles"

    def introduce(self):
        return f"Hi! I'm {self.name} 👋"

me = Developer()
print(me.introduce())
print("Passionate about backend development and machine learning applications.")
```

---

`contact`

```json
{
  "contact_info": {
    "linkedin": "https://www.linkedin.com/in/anurag-bonde-38a86428b/",
    "github":   "github.com/Anuraggg-19",
    "location": "Mumbai, India"
  },
  "availability": {
    "status": "🟢 Online & Available",
    "interested_in": [
      "🤝 Problem Solving",
      "🏆 CP Discussions",
      "🚀 Cool Projects",
      "💼 Internship / Full-time Opportunities",
      "🧠 Backend & ML Collaborations"
    ],
    "response_time": "⚡ Usually within 24 hours"
  },
  "competitive_profiles": {
    "codechef": "https://www.codechef.com/users/anuragg19",
    "leetcode": "https://leetcode.com/u/anuragg19/"
  }
}
```

---

## 📊 GitHub Statistics & Analytics

| Stats | Languages |
|-------|-----------|
| ![Anurag's GitHub Stats](https://github-readme-stats.vercel.app/api?username=Anuraggg-19&show_icons=true&theme=tokyonight&hide_border=true&bg_color=1a1b27&title_color=70a5fd&icon_color=70a5fd&text_color=ffffff) | ![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Anuraggg-19&layout=compact&theme=tokyonight&hide_border=true&bg_color=1a1b27&title_color=70a5fd&text_color=ffffff) |



[![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=Anuraggg-19&theme=tokyo-night&hide_border=true&bg_color=1a1b27&color=70a5fd&line=70a5fd&point=ffffff&area=true&area_color=70a5fd)](https://github.com/Anuraggg-19)

---



)

---

> *"Building stuff that matters, one commit at a time."* 🚀
